...
export const useBlockchain = () => {
    ...
    const chainId = !hostChainId || hostChainId === '_unset' ? globalChainId : hostChainId;
    ...
    const getCurrentChainIds = async () => {
        try {
            ...
            if (tabs[0] && tabs[0].url?.startsWith('https://')) {
                ...
                setHost(url.host);
                const hostChainIdRes = await browser.storage.local.get(`chainId_${url.host}`);
                if (
    ...
    const getUserData = async () => {
        ...
        const identityRes = await window.point.identity.ownerToIdentity({owner: address});
    ...
    const handleHostNetworkChange = async (e: SelectChangeEvent) => {
        ...
        if (e.target.value === '_unset') {
            ...
            await browser.storage.local.set({[`chainId_${host}`]: e.target.value});
        }
...
export const BlockchainContext = createContext<ReturnType<typeof useBlockchain>>(
